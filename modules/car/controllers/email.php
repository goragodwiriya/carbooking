<?php
/**
 * @filesource modules/car/controllers/email.php
 *
...
{
    /**
     * Send notification from car reservation number
     *
     * @param int $id
     *
     * @return string
    ...
    private static function send($order)
    {
        ...
        if (self::$cfg->demo_mode) {
            ...
            $where = [
                // Transaction maker
                ['U.id', [$order['member_id'], $order['chauffeur_id']]],
                // Admin
                ['U.status', 1]
            ];
        ...
        // Email message
        $msg = Language::trans(\Car\View\View::render($order, true));
        $ret = [];
        ...
        if (self::$cfg->noreply_email != '') {
            // email subject
            $subject = '['.self::$cfg->web_title.'] '.Language::get('Book a car').' '.Language::get('BOOKING_STATUS', '', $order['status']);
