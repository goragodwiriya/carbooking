    ...
    if (empty($_POST['username']) || empty($_POST['password'])) {
        ...
        if (!$error) {
            ...
            try {
                ...
                // car_reservation
                // =========================================================
                $table_reservation = $db_config['prefix'].'_car_reservation';
                ...
                if ($db->isColumnType($table_reservation, 'comment', 'text')) {
                    ...
                    $content[] = '<li class="correct">car_reservation: แก้ไข comment เป็น TEXT</li>';
                }
                if ($db->isColumnType($table_reservation, 'detail', 'text')) {
                    ...
                    $content[] = '<li class="correct">car_reservation: แก้ไข detail เป็น TEXT NOT NULL</li>';
                }
                if ($db->isColumnType($table_reservation, 'reason', 'text')) {
                    ...
                    $content[] = '<li class="correct">car_reservation: แก้ไข reason เป็น TEXT</li>';
                }
                // swap index status → idx_vehicle_availability + member_id(member_id, created_at)
                if ($db->indexExists($table_reservation, 'status') && !$db->indexExists($table_reservation, 'idx_vehicle_availability')) {
                    $db->query("DROP INDEX `status` ON `$table_reservation`");
                    $db->query("ALTER TABLE `$table_reservation` ADD INDEX `idx_vehicle_availability` (`vehicle_id`, `status`, `approve`, `begin`, `end`)");
                    $content[] = '<li class="correct">car_reservation: เพิ่ม index idx_vehicle_availability</li>';
                }
                if (!$db->indexExists($table_reservation, 'member_id')) {
                    ...
                    $content[] = '<li class="correct">car_reservation: เพิ่ม index member_id(member_id, created_at)</li>';
                }

                $content[] = '<li class="correct">car_reservation อัปเกรดสำเร็จ</li>';

                // =========================================================
                // car_reservation_data
                // =========================================================
                $table_reservation_data = $db_config['prefix'].'_car_reservation_data';

                if ($db->indexExists($table_reservation_data, 'reservation_id')) {
                    $db->query("ALTER TABLE `$table_reservation_data` DROP INDEX `reservation_id`");
                }
                if (!$db->indexExists($table_reservation_data, 'idx_reservation')) {
                    $db->query("ALTER TABLE `$table_reservation_data` ADD UNIQUE `idx_reservation` (`reservation_id`, `name`) USING BTREE;");
                }

                $content[] = '<li class="correct">car_reservation_data อัปเกรดสำเร็จ</li>';

                // =========================================================
                // vehicles
                // =========================================================
                $table_vehicles = $db_config['prefix'].'_vehicles';

                if ($db->isColumnType($table_vehicles, 'color', 'varchar(20)')) {
                    $db->query("UPDATE `$table_vehicles` SET `color` = '' WHERE `color` IS NULL");
                    $db->query("ALTER TABLE `$table_vehicles` CHANGE `color` `color` VARCHAR(20) NOT NULL DEFAULT ''");
                    $content[] = '<li class="correct">vehicles: แก้ไข color เป็น NOT NULL</li>';
                }
                if ($db->isColumnType($table_vehicles, 'detail', 'text')) {
                    $db->query("UPDATE `$table_vehicles` SET `detail` = '' WHERE `detail` IS NULL");
                    $db->query("ALTER TABLE `$table_vehicles` CHANGE `detail` `detail` TEXT NOT NULL");
                    $content[] = '<li class="correct">vehicles: แก้ไข detail เป็น TEXT NOT NULL</li>';
                }
                if ($db->isColumnType($table_vehicles, 'number', 'varchar(20)')) {
                    $db->query("UPDATE `$table_vehicles` SET `number` = '' WHERE `number` IS NULL");
                    $db->query("ALTER TABLE `$table_vehicles` CHANGE `number` `number` VARCHAR(20) NOT NULL DEFAULT ''");
                    $content[] = '<li class="correct">vehicles: แก้ไข number เป็น NOT NULL</li>';
                }
                // migrate published → is_active
                if (!$db->fieldExists($table_vehicles, 'is_active')) {
                    $db->query("ALTER TABLE `$table_vehicles` ADD `is_active` TINYINT(1) NULL");
                    if ($db->fieldExists($table_vehicles, 'published')) {
                        $db->query("UPDATE `$table_vehicles` SET `is_active` = `published`");
                    } else {
                        $db->query("UPDATE `$table_vehicles` SET `is_active` = 1");
                    }
                    $db->query("ALTER TABLE `$table_vehicles` MODIFY `is_active` TINYINT(1) NOT NULL DEFAULT 1");
                    $content[] = '<li class="correct">vehicles: เพิ่ม is_active</li>';
                }
                if ($db->fieldExists($table_vehicles, 'published')) {
                    $db->query("ALTER TABLE `$table_vehicles` DROP COLUMN `published`");
                    $content[] = '<li class="correct">vehicles: ลบ published</li>';
                }
                $content[] = '<li class="correct">vehicles อัปเกรดสำเร็จ</li>';

                // =========================================================
                // vehicles_meta
                // =========================================================
                $table_vehicles_meta = $db_config['prefix'].'_vehicles_meta';
                if ($db->indexExists($table_vehicles_meta, 'vehicle_id')) {
                    $db->query("ALTER TABLE `$table_vehicles_meta` DROP INDEX `vehicle_id`;");
                }
                if (!$db->indexExists($table_vehicles_meta, 'idx_vehicle_meta')) {
                    $db->query("ALTER TABLE `$table_vehicles_meta` ADD INDEX `idx_vehicle_meta` (`vehicle_id`, `name`) USING BTREE;");
                }
                $content[] = '<li class="correct">vehicles_meta อัปเกรดสำเร็จ</li>';
